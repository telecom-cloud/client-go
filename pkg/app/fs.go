...
import (
	...
	"github.com/telecom-cloud/client-go/internal/nocopy"
...
func ServeFile(ctx *RequestContext, path string) {
	...
	if len(path) == 0 || path[0] != '/' {
		...
		if path, err = filepath.Abs(path); err != nil {
			logger.SystemLogger().Errorf("Cannot resolve path=%q to absolute file error=%s", path, err)
...
func (h *fsHandler) handleRequest(c context.Context, ctx *RequestContext) {
	...
	if n := bytes.IndexByte(path, 0); n >= 0 {
		logger.SystemLogger().Errorf("Cannot serve path with nil byte at position=%d, path=%q", n, path)
	...
	if h.pathRewrite != nil {
		...
		if n := bytes.Index(path, bytestr.StrSlashDotDotSlash); n >= 0 {
			logger.SystemLogger().Errorf("Cannot serve path with '/../' at position=%d due to security reasons, path=%q", n, path)
			ctx.AbortWithMsg("Internal Server Error", consts.StatusInternalServerError)
			return
		}
	}
	...
	if !ok {
		...
		if mustCompress && err == errNoCreatePermission {
			logger.SystemLogger().Errorf("Insufficient permissions for saving compressed file for path=%q. Serving uncompressed file. "+
		...
		if err == errDirIndexRequired {
			...
			if err != nil {
				logger.SystemLogger().Errorf("Cannot open dir index, path=%q, error=%s", filePath, err)
				ctx.AbortWithMsg("Directory index is forbidden", consts.StatusForbidden)
				return
			}
		} else if err != nil {
			logger.SystemLogger().Errorf("Cannot open file=%q, error=%s", filePath, err)
	...
	r, err := ff.NewReader()
	if err != nil {
		logger.SystemLogger().Errorf("Cannot obtain file reader for path=%q, error=%s", path, err)
		ctx.AbortWithMsg("Internal Server Error", consts.StatusInternalServerError)
		return
	}
	...
	if h.acceptByteRange {
		...
		if len(byteRange) > 0 {
			...
			if err != nil {
				r.(io.Closer).Close()
				logger.SystemLogger().Errorf("Cannot parse byte range %q for path=%q,error=%s", byteRange, path, err)
			...
			if err = r.(byteRangeUpdater).UpdateByteRange(startPos, endPos); err != nil {
				r.(io.Closer).Close()
				logger.SystemLogger().Errorf("Cannot seek byte range %q for path=%q, error=%s", byteRange, path, err)
				ctx.AbortWithMsg("Internal Server Error", consts.StatusInternalServerError)
				return
			}
	...
	if !ctx.IsHead() {
		...
		if rc, ok := r.(io.Closer); ok {
			if err := rc.Close(); err != nil {
				logger.SystemLogger().Errorf("Cannot close file reader: error=%s", err)
				ctx.AbortWithMsg("Internal Server Error", consts.StatusInternalServerError)
				return
			}
		}

...
func TestRouteStaticFile(t *testing.T) {
	...
	_, err = f.WriteString("Crafter Web Framework")
	assert.Nil(t, err)
	f.Close()
	...
	assert.DeepEqual(t, consts.StatusOK, w.Code)
	assert.DeepEqual(t, "Crafter Web Framework", w.Body.String())
...
func TestRouterMiddlewareAndStatic(t *testing.T) {
	...
	static := router.Group("/", func(c context.Context, ctx *app.RequestContext) {
		...
		ctx.Response.Header.Set("X-Crafter", "Crafter Framework")
	})
	static.Static("/", "./")
	...
	assert.DeepEqual(t, "Crafter Framework", w.Header().Get("x-Crafter"))
}

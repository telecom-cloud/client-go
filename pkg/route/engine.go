...
import (
	...
	"github.com/telecom-cloud/client-go/pkg/common/binding"
	...
	"github.com/telecom-cloud/client-go/pkg/protocol/http1/factory"
...
func (engine *Engine) Shutdown(ctx context.Context) (err error) {
	...
	defer func() {
		...
		select {
		case <-ctx.Done():
			logger.SystemLogger().Infof("Execute OnShutdownHooks timeout: error=%v", ctx.Err())
			return
		case <-ch:
			logger.SystemLogger().Info("Execute OnShutdownHooks finish")
			return
		}
	}()
	...
	if opt := engine.options; opt != nil && opt.Registry != nil {
		if err = opt.Registry.Deregister(opt.RegistryInfo); err != nil {
			logger.SystemLogger().Errorf("Deregister error=%v", err)
			return err
		}
	}
...
func (engine *Engine) listenAndServe() error {
	logger.SystemLogger().Infof("Using network library=%s", engine.GetTransporterName())
	...
	if tlsConn, ok := conn.(network.ConnTLSer); ok {
		if engine.options.ReadTimeout > 0 {
			if err := conn.SetReadTimeout(engine.options.ReadTimeout); err != nil {
				logger.SystemLogger().Errorf("BUG: error in SetReadDeadline=%s: error=%s", engine.options.ReadTimeout, err)
			}
		}
...
func errProcess(conn io.Closer, err error) {
	...
	// other errors
	logger.SystemLogger().Errorf(logger.EngineErrorFormat, err.Error(), rip)
}
...
func (engine *Engine) Serve(c context.Context, conn network.Conn) (err error) {
	...
	if engine.options.H2C {
		...
		logger.SystemLogger().Warn("HTTP2 server is not loaded, request is going to fallback to HTTP1 server")
	}
...
func debugPrintRoute(httpMethod, absolutePath string, handlers app.HandlersChain) {
	...
	logger.SystemLogger().Debugf("Method=%-6s absolutePath=%-25s --> handlerName=%s (num=%d handlers)", httpMethod, absolutePath, handlerName, nuHandlers)
}
...
func (engine *Engine) LoadHTMLGlob(pattern string) {
	...
	if engine.options.AutoReloadRender {
		...
		if err != nil {
			logger.SystemLogger().Errorf("LoadHTMLGlob: %v", err)
			return
		}

...
func TestCheckWriteHeaderCode(t *testing.T) {
	...
	assert.True(t, strings.Contains(buffer.String(), "[Warn] CRAFTER: Invalid StatusCode code"))
	buffer.Reset()
	checkWriteHeaderCode(600)
	assert.True(t, strings.Contains(buffer.String(), "[Warn] CRAFTER: Invalid StatusCode code"))
	buffer.Reset()
	checkWriteHeaderCode(100)
	assert.False(t, strings.Contains(buffer.String(), "[Warn] CRAFTER: Invalid StatusCode code"))
	buffer.Reset()
...
func TestSetTrailerWithROString(t *testing.T) {
	h := &RequestHeader{}
	h.Add(consts.HeaderTrailer, "foo,bar,crafter")
	assert.DeepEqual(t, "Foo, Bar, Crafter", h.Get(consts.HeaderTrailer))

	h1 := &ResponseHeader{}
	h1.Add(consts.HeaderTrailer, "foo,bar,crafter")
	assert.DeepEqual(t, "Foo, Bar, Crafter", h1.Get(consts.HeaderTrailer))
}

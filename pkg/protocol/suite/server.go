...
func (c *Config) Add(protocol string, factory interface{}) {
	...
	case ServerFactory:
		if fac := c.configMap[protocol]; fac != nil {
			logger.SystemLogger().Warnf("ServerFactory of protocol: %s will be overridden by customized function", protocol)
		}
		c.configMap[protocol] = factory
	case StreamServerFactory:
		if fac := c.streamConfigMap[protocol]; fac != nil {
			logger.SystemLogger().Warnf("StreamServerFactory of protocol: %s will be overridden by customized function", protocol)
		}
		c.streamConfigMap[protocol] = factory
	default:
		logger.SystemLogger().Fatalf("Unsupported factory type: %T", factory)
	}
}
